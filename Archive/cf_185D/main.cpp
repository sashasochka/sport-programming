...
void solve() {
    ...
    for (int i = 0; i < m; ++i) {
        cin >> cats[i].h;
        cats[i].d = d[cats[i].h - 1];
        cin >> cats[i].t;
    }
    sort(cats.begin(), cats.end());
    vi tg(m);
    for (int i = 0; i < m; ++i) {
        if (cats[i].visited) continue;
        int t = max(0, cats[i].t - cats[i].d);
        for (int j = 0; j < m; ++j) {
            if (t + cats[j].d >= cats[j].t) {
                cats[j].visited = true;
                tg[j] = t + cats[j].d - cats[j].t;
            }
        }
    }
    cout << accumulate(tg.begin(), tg.end(), 0) << endl;
}
